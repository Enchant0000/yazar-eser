...
function SikliQuizScreen({ kartlar, tumKartlar, onExit, zamanLimit, onTestBitti }) {
  ...
  const [mevcutSeri, setMevcutSeri] = useState(0);
  const [enYuksekSeri, setEnYuksekSeri] = useState(0);
  ...
  const handleSec = (sikIndex) => {
    ...
    if (dogru) {
      ...
      setMevcutSeri(yeniSeri);
      if (yeniSeri > enYuksekSeri) setEnYuksekSeri(yeniSeri);
    } else {
      setYanlisList(p => [...p, kart]);
      setMevcutSeri(0);
    }
  ...
  useEffect(() => {
    if (bitti && onTestBitti) {
      onTestBitti(dogruSayisi, yanlisList.length, kartlar.length, enYuksekSeri);
    }
  }, [bitti]);
...
export default function App() {
  ...
  const handleTestBitti = useCallback((dogruSayisi, yanlisSayisi, toplam, seriDogruSayisi) => {
    if (!testMeta) return;
    ...
    const sonuc = {
      ...
      seriDogruSayisi: seriDogruSayisi || 0,
    };
