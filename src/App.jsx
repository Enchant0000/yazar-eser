...
/* ─── Şıklı Quiz ──────────────────────────────────────────────────── */
function SikliQuizScreen({ kartlar, tumKartlar, onExit, zamanLimit }) {
  const [index, setIndex] = useState(0);
...
export default function App() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "linear-gradient(to bottom, #020617, #0f172a, #020617)", color: "#fff" }}>
      ...
      {quizModu && (
        <SikliQuizScreen key={quizKey} kartlar={quizKartlar} tumKartlar={tumKartlar} onExit={handleExitQuiz} zamanLimit={testZaman} />
      )}
      </>
      )}
    </div>
